	...
	_.forIn(registers, function(registerArr, registerId, obj){
		...
		obj[registerId] = registerArr.reduce((prevProps, props) => {
			...
			if (prevProps.length == 0) {
				...
				newprops.start = prevProps[prevProps.length - 1].start + prevProps[prevProps.length - 1].len;
			}
	...
	_.each(fields, function(field){
		...
		if(field.type == 'number') {
			if (field.decimals) {
				...
				fieldContent = parseFloat(fieldContent.slice(0, idxDec) + "." + fieldContent.slice(idxDec));
			} else {
...
EdiFile.prototype.edify = function(customRegisters) {
	...
	_.each(customRegisters, function(cRegister) {
		...
		var parsedLine = fieldsToString(cRegister.properties, registers[registerId]);
...
EdiFile.prototype.fieldsToString = function(properties, fields) {
	...
	_.each(fields, function(field){
		...
		if (!_.has(properties, field.name)) {
			throw new Error('Field is not present. Field:' + field.name + ' Properties:' + JSON.stringify(properties));
		}

		if (field.type == 'string') {
			if (properties[field.name].length > numberOfCharacters) {
				throw new Error('Content has more characters than the limit. Field: ' + field.name + ' Content: ' + properties[field.name]);
			}

			fieldContent = properties[field.name] + new Array(numberOfCharacters - properties[field.name].length + 1).join(' ');
			...
			var strNumber = properties[field.name].toFixed(numberOfDecimals).replace('.', '');

			if (strNumber.length > numberOfCharacters) {
				throw new Error('Content has more numbers than the limit. Field: ' + field.name + ' Content: ' + properties[field.name]);
			}

			fieldContent = new Array(numberOfCharacters - strNumber.length + 1).join('0') + strNumber;
		} else if(field.type == 'date') {
			fieldContent = moment(properties[field.name]).format(this.options.dateFormat);
