...
const {
	...
	getFileLineEnding,
} = require("../helpers/buildInsertKeys");
...
EdiFile.prototype.parseComplexFile = function (fileContent) {
	...
	_.each(
		...
		function (line) {
			...
			childPositions = updateRegisterPosition(registerId, childPositions);

			childPositions = resetRegisterPositions(registerId, childPositions, tree);

			let insertSequence = buildInsertKeys(registerId, childPositions, tree);

			// console.log("insert sequence: ", registerId + "|" + insertSequence);
			_.set(registers, insertSequence, parsedLine);
		}.bind(this)
	);
	// console.log("parseado ", JSON.stringify(registers));
