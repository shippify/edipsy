...
var EdiFile = function (registers, options) {
	...
	_.each(registers, function (fields) {
		...
		_.each(fields, function (field, i) {
			...
			previousFieldDecimal = parseInt(field.decimals) || 0;

			if (field.end) {
				previousFieldEnd = field.end;
			} else {
				previousFieldEnd = field.start + field.len + previousFieldDecimal - 1;
			}
		});
	});
...
EdiFile.prototype.parseComplexFile = function (fileContent) {
	...
	const headers = Object.keys(tree.nodes);
	...
	_.each(
		...
		function (line) {
			...
			if (!headers.includes(registerId)) return;
	...
	_.each(
		...
		function (field) {
			...
			if (field.end) {
				...
				const decimals = parseInt(field.decimals) || 0;

				fieldContent = line.substring(
					field.start - 1,
					field.start + field.len + decimals - 1
				);
			}
