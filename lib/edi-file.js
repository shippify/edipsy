...
EdiFile.prototype.parseComplexFile = function (fileContent) {
	...
	_.each(
		...
		function (line) {
			...
			registerPositions = updateRegisterPosition(registerId, registerPositions);

			registerPositions = resetRegisterPositions(registerId, registerPositions, tree);

			let insertSequence = buildInsertKeys(registerId, registerPositions, tree);
	...
	_.each(
		...
		function (field) {
			...
			if (field.type == "string") {
				...
				properties[field.name] = !field.decimals ? parseInt(properties[field.name]) : parseFloat(properties[field.name]);
				properties[field.name] = properties[field.name] ? properties[field.name] : 0;
				...
				return field.fields.map((sField)=>{

					if (sField.type == "string") {
						if (properties[sField.name].length > numberOfCharacters) {
							throw new Error(
								"Content has more characters than the limit. Field: " +
									sField.name +
									" Content: " +
									properties[sField.name]
							);
						}
						return fieldContent =
							properties[sField.name].padStart(sField.name.len,"0")+
							new Array(
								numberOfCharacters - properties[sField.name].length + 1
							).join(" ");
					} else if (sField.type == "number") {
						var numberOfDecimals = 0;
						if (sField.decimals) {
							numberOfDecimals = sField.decimals;
						}
						properties[sField.name] = !sField.decimals ? parseInt(properties[sField.name]) : parseFloat(properties[sField.name]);
						properties[sField.name] = properties[sField.name] ? properties[sField.name] : 0;
						
						var strNumber2 = properties[sField.name]
							.toFixed(numberOfDecimals)
							.replace(".", "");
		
						if (strNumber2.length > numberOfCharacters) {
							throw new Error(
								"Content has more numbers than the limit. Field: " +
								sField.name +
									" Content: " +
						...
						fieldContent =
							new Array(numberOfCharacters - strNumber2.length + 1).join("0") +
							strNumber2;
					} else if (field.type == "date") {
						fieldContent = moment(properties[sField.name]).format(
							this.options.dateFormat
						);
